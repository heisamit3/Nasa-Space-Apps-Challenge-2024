...
//           const signupData = { email, password, confirm_password: retypePassword }; // Ensure you match the backend expected format
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        // Handle signup logic
        if (password === retypePassword) {
          const signupData = { email, password }; // Ensure you match the backend expected format
          const response = await axios.post(`http://localhost:8000/api/signup/`, signupData);
          console.log(response.data.message); // Log success message
          // Navigate to SignUpPage for preferences
          navigate("/signup-preferences", { state: { email } });
        } else {
          alert("Passwords do not match!");
        }
      }
