...
import "../../css/AirSeaCO2MapUnifiedPage.css"; // Import custom CSS for styling
...
const AirSeaCO2MapWithDataPage: React.FC = () => {
    ...
    useMapEvents({
      click: async (event) => {
        ...
        setTimeout(async () => {
          try {
            const response = await fetch(
              "http://127.0.0.1:8000/compute_air_sea_co2_stats_view/",
              {
                method: "POST",
                headers: {
  ...
  const AirSeaEmissionsGraph = ({
    ...
    const chartOptions = {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `AirSea Emissions Trends for ${region}`,
          color: "#ff8000",
          font: {
            size: 20,
            weight: "bold",
...
export default AirSeaCO2MapWithDataPage;
